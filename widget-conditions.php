...
class ClassicPress_Widget_Conditions {
	...
	public static function widget_admin_setup() {
        ...
        wp_enqueue_script( 'widget-conditions', plugins_url( 'widget-conditions/widget-conditions.js', __FILE__ ),  array( 'jquery', 'jquery-ui-core' ), null, 'true');
		...
		$widget_conditions_data['category'][] = array( '', __( 'All category pages', 'cp-widget-conditions' ) );
		...
		$widget_conditions_data['loggedin'][] = array( 'loggedin', __( 'Logged In', 'cp-widget-conditions' ) );
		$widget_conditions_data['loggedin'][] = array( 'loggedout', __( 'Logged Out', 'cp-widget-conditions' ) );

		$widget_conditions_data['author']   = array();
		$widget_conditions_data['author'][] = array( '', __( 'All author pages', 'cp-widget-conditions' ) );
		...
		$widget_conditions_data['tag'][] = array( '', __( 'All tag pages', 'cp-widget-conditions' ) );
		...
		$widget_conditions_data['date'][] = array( '', __( 'All date archives', 'cp-widget-conditions' ) );
		$widget_conditions_data['date'][] = array( 'day', __( 'Daily archives', 'cp-widget-conditions' ) );
		$widget_conditions_data['date'][] = array( 'month', __( 'Monthly archives', 'cp-widget-conditions' ) );
		$widget_conditions_data['date'][] = array( 'year', __( 'Yearly archives', 'cp-widget-conditions' ) );

		$widget_conditions_data['page']   = array();
		$widget_conditions_data['page'][] = array( 'front', __( 'Front page', 'cp-widget-conditions' ) );
		$widget_conditions_data['page'][] = array( 'posts', __( 'Posts page', 'cp-widget-conditions' ) );
		$widget_conditions_data['page'][] = array( 'archive', __( 'Archive page', 'cp-widget-conditions' ) );
		$widget_conditions_data['page'][] = array( '404', __( '404 error page', 'cp-widget-conditions' ) );
		$widget_conditions_data['page'][] = array( 'search', __( 'Search results', 'cp-widget-conditions' ) );
		...
		$widget_conditions_data['page'][] = array( __( 'Post type:', 'cp-widget-conditions' ), $widget_conditions_post_types );

		$widget_conditions_data['page'][] = array( __( 'Post type Archives:', 'cp-widget-conditions' ), $widget_conditions_post_type_archives );
		...
		$widget_conditions_data['page'][] = array( __( 'Static page:', 'cp-widget-conditions' ), $static_pages );

		$widget_conditions_data['taxonomy']   = array();
		$widget_conditions_data['taxonomy'][] = array( '', __( 'All taxonomy pages', 'cp-widget-conditions' ) );
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			$widget_conditions_terms[] = array( $taxonomy->name, __( 'All pages', 'cp-widget-conditions' ) );
						...
						<div class="condition" data-rule-major="<?php echo esc_attr( $rule['major'] ); ?>" data-rule-minor="<?php echo esc_attr( $rule['minor'] ); ?>" data-rule-has-children="<?php echo esc_attr( $rule['has_children'] ); ?>">
							<div class="selection alignleft">
								<select class="conditions-rule-major" name="conditions[rules_major][]">
									<option value="" <?php selected( '', $rule['major'] ); ?>><?php echo esc_html_x( '-- Select --', 'Used as the default option in a dropdown list', 'cp-widget-conditions' ); ?></option>
									<option value="category" <?php selected( 'category', $rule['major'] ); ?>><?php esc_html_e( 'Category', 'cp-widget-conditions' ); ?></option>
									<option value="author" <?php selected( 'author', $rule['major'] ); ?>><?php echo esc_html_x( 'Author', 'Noun, as in: "The author of this post is..."', 'cp-widget-conditions' ); ?></option>

									<?php if ( ! ( defined( 'IS_WPCOM' ) && IS_WPCOM ) ) { // this doesn't work on .com because of caching ?>
										<option value="loggedin" <?php selected( 'loggedin', $rule['major'] ); ?>><?php echo esc_html_x( 'User', 'Noun', 'cp-widget-conditions' ); ?></option>
										<option value="role" <?php selected( 'role', $rule['major'] ); ?>><?php echo esc_html_x( 'Role', 'Noun, as in: "The user role of that can access this widget is..."', 'cp-widget-conditions' ); ?></option>
									<?php } ?>

									<option value="tag" <?php selected( 'tag', $rule['major'] ); ?>><?php echo esc_html_x( 'Tag', 'Noun, as in: "This post has one tag."', 'cp-widget-conditions' ); ?></option>
									<option value="date" <?php selected( 'date', $rule['major'] ); ?>><?php echo esc_html_x( 'Date', 'Noun, as in: "This page is a date archive."', 'cp-widget-conditions' ); ?></option>
									<option value="page" <?php selected( 'page', $rule['major'] ); ?>><?php echo esc_html_x( 'Page', 'Example: The user is looking at a page, not a post.', 'cp-widget-conditions' ); ?></option>
									<?php if ( get_taxonomies( array( '_builtin' => false ) ) ) : ?>
										<option value="taxonomy" <?php selected( 'taxonomy', $rule['major'] ); ?>><?php echo esc_html_x( 'Taxonomy', 'Noun, as in: "This post has one taxonomy."', 'cp-widget-conditions' ); ?></option>
									<?php endif; ?>
								</select>

								<?php _ex( 'is', 'Widget Visibility: {Rule Major [Page]} is {Rule Minor [Search results]}', 'cp-widget-conditions' ); ?>
								...
								if ( ! $rule['has_children'] ) {
									...
									<label>
										...
										<?php echo esc_html_x( 'Include children', 'Checkbox on Widget Visibility if children of the selected page should be included in the visibility rule.', 'cp-widget-conditions' ); ?>
									</label>
								</span>
							</div>
							...
							<div class="condition-control">
								<span class="condition-conjunction">
									<?php echo esc_html_x( 'or', 'Shown between widget visibility conditions.', 'cp-widget-conditions' ); ?>
								</span>
								<span class="condition-intersection">
									<?php echo esc_html_x( 'and', 'Shown between widget visibility conditions.', 'cp-widget-conditions' ); ?>
								</span>
								<div class="actions alignright">
									<a href="#" class="delete-condition dashicons dashicons-no"><?php esc_html_e( 'Delete', 'cp-widget-conditions' ); ?></a><a href="#" class="add-condition dashicons dashicons-plus"><?php esc_html_e( 'Add', 'cp-widget-conditions' ); ?></a>
								</div>
							</div>
				...
				<div class="conditions">
					<div class="condition-top">
						<label>
							...
							<?php esc_html_e( 'Match all conditions', 'cp-widget-conditions' ); ?>
						</label>
					</div><!-- .condition-top -->
				</div><!-- .conditions -->
